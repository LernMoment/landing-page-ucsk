#!/bin/bash
# Skript zum Deployment von Landing Page UCsK
...
STAGING_SITE_BUILD_COMMAND="jekyll build --config _config.yml,_config-staging.yml"
STAGING_ZIPPED_SITE=deployment/ucsk_lp-site-staging2.tgz
STAGING_SERVER_AREA=1und1:staging2

#Informationen für Production
PRODUCTION_SITE_BUILD_COMMAND="jekyll build --config _config.yml"
PRODUCTION_ZIPPED_SITE=deployment/ucsk_lp-site-production.tgz
...
docker run --rm -v $(pwd):/srv/jekyll -e "TZ=Europe/Berlin" -e "LANG=C.UTF-8" --entrypoint="/bin/bash" jekyll/jekyll -c "$SITE_BUILD_COMMAND"
